...
// NEW: Historical Data Migration Endpoints
router.post('/historical-migration/start', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { HistoricalDataMigration } = require('../services/historicalDataMigration');
    const migrationService = new HistoricalDataMigration();
    ...
    console.log('🚀 Admin triggered historical data migration...');
    console.log(`📅 Date range: ${startDate} to ${endDate} (${daysDiff} days)`);
    console.log(`👥 Creators: ${specificCreatorIds ? specificCreatorIds.length + ' specific' : 'all active'}`);
    console.log(`🔍 Mode: ${dryRun ? 'DRY RUN' : 'LIVE'}`);

    if (daysDiff > 30) {
      // Large migration - run in background
      setImmediate(async () => {
        try {
          const results = await migrationService.mapAllCreatorsHistoricalData({
            startDate: start,
            endDate: end,
            specificCreatorIds,
            dryRun,
          ...
          console.log('✅ Background historical migration completed:', {
            successful: results.creators.successful,
            failed: results.creators.failed,
            records: results.data.totalRecords,
            duration: results.performance.totalDuration
          });
        } catch (error) {
          console.error('❌ Background historical migration failed:', error.message);
        }
      });

      res.json({
        success: true,
        message: `Historical data migration started for ${daysDiff} days (running in background)`,
        estimatedDuration: `${Math.ceil(daysDiff * 0.5)} minutes`,
        mode: dryRun ? 'DRY_RUN' : 'LIVE',
        background: true
      });
    } else {
      // Small migration - run synchronously
      const results = await migrationService.mapAllCreatorsHistoricalData({
        startDate: start,
        endDate: end,
        specificCreatorIds,
      ...
      res.json({
        ...
        results: {
          ...
          data: {
            ...
            impactApiCalls: results.data.impactApiCalls
          },
          performance: {
            totalDuration: results.performance.totalDuration,
            averageTimePerCreator: results.performance.averageTimePerCreator
          },
          errors: results.data.errors.slice(0, 10) // Show first 10 errors only
        },
        mode: dryRun ? 'DRY_RUN' : 'LIVE',
        background: false
      });
    ...
    console.error('❌ Historical data migration error:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to start historical data migration',
      error: error.message
    });
  }
});
...
router.get('/historical-migration/status', requireAuth, requireAdmin, async (req, res) => {
  try {
    // Check recent DailyAnalytics records to show current state
    const recentRecords = await prisma.dailyAnalytics.findMany({
      take: 10,
    ...
    const [totalRecords, oldestRecord, newestRecord, creatorCounts] = await Promise.all([
      prisma.dailyAnalytics.count(),
      prisma.dailyAnalytics.findFirst({
        orderBy: { date: 'asc' },
        select: { date: true, creator: { select: { name: true } } }
      }),
      prisma.dailyAnalytics.findFirst({
        orderBy: { date: 'desc' },
        select: { date: true, creator: { select: { name: true } } }
    ...
    res.json({
      ...
      status: {
        totalRecords,
        creatorsWithData: creatorCounts.length,
        dateRange: {
        ...
        recentActivity: recentRecords.map(record => ({
          ...
          metrics: {
            sales: record.commissionableSales,
            commissions: record.commissionEarned,
    ...
    console.error('❌ Historical migration status error:', error);
    res.status(500).json({
      success: false,
...
// Full Historical Backfill - Map all creators' historical data from Impact.com
router.post('/analytics/full-historical-backfill', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { DailyAnalyticsService } = require('../services/dailyAnalyticsService');
    const analyticsService = new DailyAnalyticsService();
    ...
    console.log('🗺️ [Full Historical Backfill] Admin triggered comprehensive historical data mapping...');
    console.log(`📅 Date range: ${startDate} to ${endDate}`);
    console.log(`👥 All creators: ${allCreators}`);
    console.log(`🔄 Setup auto-collection: ${setupAutoDailyCollection}`);
    ...
    console.log(`👥 [Full Historical Backfill] Found ${creators.length} active creators to process`);

    if (creators.length === 0) {
      return res.json({
        success: false,
        message: 'No active creators found to process'
      });
    }
    ...
    const end = new Date(endDate);
    const daysDiff = Math.ceil((end - start) / (24 * 60 * 60 * 1000));
    const estimatedMinutes = Math.ceil((creators.length * daysDiff) / 30); // ~30 records per minute

    // Return immediate response and start processing in background
    res.json({
      success: true,
      message: 'Full historical backfill started - mapping all creator data from Impact.com API',
      ...
      status: 'processing',
      features: {
        historicalDataMapping: true,
        pointInTimeSnapshots: true,
        automaticDailyUpdates: setupAutoDailyCollection,
        productionSafe: true
      }
    });
    ...
    setImmediate(async () => {
      try {
        console.log('🚀 [Full Historical Backfill] Starting background processing...');
        ...
        const batchSize = 3; // Conservative batch size for production safety
        
        for (let i = 0; i < creators.length; i += batchSize) {
          const batch = creators.slice(i, i + batchSize);
          console.log(`📊 [Full Historical Backfill] Processing batch ${Math.floor(i/batchSize) + 1}/${Math.ceil(creators.length/batchSize)}`);

          const batchPromises = batch.map(async (creator) => {
            try {
              console.log(`🔍 [Full Historical Backfill] Processing ${creator.name} (${creator.email})`);
              
              // Use existing backfill method for each creator
              const creatorResults = await analyticsService.backfillHistoricalData(start, end, creator.id);
              
              console.log(`✅ [Full Historical Backfill] Completed ${creator.name}: ${creatorResults.successful} days processed`);
              
              results.successful++;
              results.totalRecordsCreated += creatorResults.successful;
              
              return { success: true, creator: creator.name, records: creatorResults.successful };
              
            } catch (error) {
              console.error(`❌ [Full Historical Backfill] Failed for ${creator.name}:`, error.message);
              results.failed++;
              results.errors.push(`${creator.name}: ${error.message}`);
              
              return { success: false, creator: creator.name, error: error.message };
            }
          });

          await Promise.all(batchPromises);

          // Rate limiting between batches (production safety)
          if (i + batchSize < creators.length) {
            console.log('⏳ [Full Historical Backfill] Rate limiting... waiting 3 seconds');
            await new Promise(resolve => setTimeout(resolve, 3000));
          }
        }

        // Setup automatic daily collection if requested
        if (setupAutoDailyCollection) {
          try {
            console.log('🔄 [Full Historical Backfill] Setting up automatic daily collection...');
            
            // This would typically involve setting up a cron job or scheduled task
            // For now, we'll just log that it's ready
            console.log('✅ [Full Historical Backfill] System ready for automatic daily updates');
            console.log('💡 [Full Historical Backfill] Use "Collect Yesterday" button daily or set up cron job');
            
          } catch (setupError) {
            console.error('⚠️ [Full Historical Backfill] Auto-collection setup warning:', setupError.message);
          }
        }
        ...
        console.log('🎉 [Full Historical Backfill] COMPLETED!');
        console.log(`📊 Results: ${results.successful} creators successful, ${results.failed} failed`);
        console.log(`📈 Created ${results.totalRecordsCreated} historical records`);
        console.log(`⏱️ Duration: ${duration} minutes`);
        
        if (results.errors.length > 0) {
          console.log('⚠️ [Full Historical Backfill] Errors encountered:');
          results.errors.forEach(error => console.log(`   - ${error}`));
        }

      } catch (backgroundError) {
        console.error('❌ [Full Historical Backfill] Background processing failed:', backgroundError.message);
      }
    });

  } catch (error) {
    console.error('❌ [Full Historical Backfill] Setup error:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to start full historical backfill',
      error: error.message
    });
  }
});
